...
public class EIUtils {
	...
	// perch� y � diventata i2 ?????
	public double I(Multimap<Integer, Individual> x, Description y, Set<Individual> individuals) {
		int numInd = individuals.size();

		Map<Integer, Double> numX = Maps.newHashMap();
		Map<LogicValue, Double> numY = Maps.newHashMap();

		Table<Integer, LogicValue, Double> prXY = HashBasedTable.create();
		...
		Map<Individual, Integer> map = Maps.newHashMap();
		for (Entry<Integer, Individual> e : x.entries()) {
		...
		for (Individual i : individuals) {
			...
			Integer rate = map.get(i);
			
			//infValY = (infValY == LogicValue.FALSE ? LogicValue.UNKNOWN : infValY);
			numX.put(rate, numX.get(rate) + 1.0);
			ZX += 1.0;
			
			numY.put(infValY, numY.get(infValY) + 1.0);
			ZC += 1.0;
			
			prXY.put(rate, infValY, prXY.get(rate, infValY) + 1.0);
			ZXC += 1.0;
		}
